...
namespace JigLibX.Physics {
    ...
    public class PhysicsSystem {
        ...
        public void AddBody(Body body) {
            ...
            if (collisionSystem != null && body.CollisionSkin != null) collisionSystem.AddCollisionSkin(body.CollisionSkin);
        }
        ...
        public bool RemoveBody(Body body) {
            if (collisionSystem != null && body.CollisionSkin != null) collisionSystem.RemoveCollisionSkin(body.CollisionSkin);
        ...
        private void DoShockStep(float dt) {
                ...
                for (int ibody = 0; ibody < numBodies; ++ibody) {
                    ...
                    if (!body.Immovable && body.DoShockProcessing) {
                        ...
                        if (skin != null) {
                            ...
                            if (0 == numColls || !body.IsActive) { body.InternalSetImmovable(); } else {
            ...
            for (int i = 0; i < numBodies; ++i) bodies[i].InternalRestoreImmovable();
        }
        ...
        private void GetAllExternalForces(float dt) {
            int numBodies = bodies.Count;
            for (int i = 0; i < numBodies; ++i) bodies[i].AddExternalForces(dt);

            int numControllers = controllers.Count;
            for (int i = 0; i < numControllers; ++i) controllers[i].UpdateController(dt);
        }
        ...
        private void UpdateAllPositions(float dt) {
            ...
            for (int i = 0; i < numBodies; ++i) activeBodies[i].UpdatePositionWithAux(dt);
        }
        ...
        private void NotifyAllPostPhysics(float dt) {
            int numBodies = bodies.Count;
            for (int i = 0; i < numBodies; ++i) bodies[i].PostPhysics(dt);
        }
        ...
        private bool ProcessCollision(CollisionInfo collision, float dt, bool firstContact) {
            ...
            for (int pos = 0; pos < collision.NumCollPts; ++pos) {
                ...
                if (body1 != null) body1.ApplyNegativeBodyWorldImpulse(ref impulse, ref ptInfo.Info.R1);
                ...
                if (tangentSpeed > minVelForProcessing) {
                    ...
                    if (denominator > JiggleMath.Epsilon) {
                        ...
                        float impulseFromNormalImpulse = collision.MatPairProperties.StaticFriction * normalImpulse;
                        ...
                        if (body1 != null) body1.ApplyNegativeBodyWorldImpulse(ref T, ref ptInfo.Info.R1);
                    }
                } // end of friction
            }
            ...
            for (int pos = collision.NumCollPts; pos-- != 0;) {
                ...
                if (doCorrection) {
                    ...
                    if (System.Math.Abs(deltaVel) > minVelForProcessing) {
                        ...
                        if (body1 != null) body1.ApplyNegativeBodyWorldImpulseAux(ref impulse, ref ptInfo.Info.R1);
                        //prepare our return value
                        gotOne = true;
                    }
                }
                ...
                if (ptInfo.AccumulatedNormalImpulse > 0.0f) {
                    ...
                    if (tangentSpeed > minVelForProcessing) {
                        ...
                        if (denominator > JiggleMath.Epsilon) {
                            ...
                            Vector3.Subtract(ref ptInfo.AccumulatedFrictionImpulse, ref origAccumulatedFrictionImpulse, out actualFrictionImpulse);

                            body0.ApplyBodyWorldImpulse(ref actualFrictionImpulse, ref ptInfo.Info.R0);
                            if (body1 != null) body1.ApplyNegativeBodyWorldImpulse(ref actualFrictionImpulse, ref ptInfo.Info.R1);
                        }
                    } // end of friction
                }
            }
        ...
        private unsafe bool ProcessCollisionCombined(CollisionInfo collision, float dt, bool firstContact) {
            ...
            if (normalVel2 < avMinSeparationVel) {
                ...
                if (deltaVel > minVelForProcessing) {
                    float denominator = 0.0f;
                    if (!body0.Immovable) denominator = body0.InverseMass + Vector3.Dot(N, Vector3.Cross(Vector3.TransformNormal(Vector3.Cross(R0, N), body0.WorldInvInertia), R0));
                    if (body1 != null && !body1.Immovable) denominator += body1.InverseMass + Vector3.Dot(N, Vector3.Cross(Vector3.TransformNormal(Vector3.Cross(R1, N), body1.WorldInvInertia), R1));
                    ...
                    if (body1 != null) body1.ApplyNegativeWorldImpulse(impulse, avPos);
                }
            }
            ...
            for (pos = collision.NumCollPts; pos-- != 0;) {
                ...
                Vector3 vrNew = body1 != null ? body0.GetVelocity(ptInfo.Info.R0) - body1.GetVelocity(ptInfo.Info.R1) : body0.GetVelocity(ptInfo.Info.R0);
                ...
                if (tangentSpeed > minVelForProcessing) {
                    ...
                    if (!body0.Immovable) denominator = body0.InverseMass + Vector3.Dot(T, Vector3.Cross(Vector3.TransformNormal(Vector3.Cross(ptInfo.Info.R0, T), body0.WorldInvInertia), ptInfo.Info.R0));

                    if (body1 != null && !body1.Immovable) denominator += body1.InverseMass + Vector3.Dot(T, Vector3.Cross(Vector3.TransformNormal(Vector3.Cross(ptInfo.Info.R1, T), body1.WorldInvInertia), ptInfo.Info.R1));

                    if (denominator > JiggleMath.Epsilon) {
                        float impulseToReverse = tangentSpeed / denominator;
                        float impulseFromNormalImpulse = collision.MatPairProperties.StaticFriction * ptNormalImpulse;
                        float frictionImpulse;
        ...
        private bool ProcessCollisionsForShock(CollisionInfo collision, float dt) {
            ...
            for (pos = 0; pos < collision.NumCollPts; ++pos) {
                ...
                if (body0 != null) normalVel = Vector3.Dot(body0.GetVelocity(ptInfo.Info.R0), N) + Vector3.Dot(body0.GetVelocityAux(ptInfo.Info.R0), N);
                if (body1 != null) normalVel -= Vector3.Dot(body1.GetVelocity(ptInfo.Info.R1), N) + Vector3.Dot(body1.GetVelocityAux(ptInfo.Info.R1), N);
                ...
                if (body0 != null) body0.ApplyBodyWorldImpulseAux(ref actualImpulse, ref ptInfo.Info.R0);
                if (body1 != null) body1.ApplyNegativeBodyWorldImpulseAux(ref actualImpulse, ref ptInfo.Info.R1);
            }
        ...
        private void PreProcessCollision(CollisionInfo collision, float dt) {
            ...
            for (int pos = 0; pos < collision.NumCollPts; ++pos) {
                ...
                // movable
                if (body0.Immovable) { ptInfo.Denominator = 0.0f; } else {
                ...
                if (ptInfo.Info.InitialPenetration > allowedPenetration) { ptInfo.MinSeparationVel = (ptInfo.Info.InitialPenetration - allowedPenetration) / timescale; } else {
        ...
        private void PreProcessCollisionAccumulated(CollisionInfo collision, float dt) {
            ...
            for (int pos = 0; pos < collision.NumCollPts; ++pos) {
                ...
                for (int i = 0; i < count; i++) {
                    if (!(bp.BodyA == catchedContacts[i].Pair.BodyA && bp.BodyB == catchedContacts[i].Pair.BodyB)) continue;
                    ...
                    if (distSq < bestDistSq) {
                        ...
                        if (catchedContacts[i].Pair.BodyA != collision.SkinInfo.Skin0.Owner) ptInfo.AccumulatedFrictionImpulse *= -1;
                    }
                }
        ...
        private void HandleAllConstraints(float dt, int iter, bool forceInelastic) {
            ...
            for (int i = 0; i < numConstraints; ++i) constraints[i].PreApply(dt);
            ...
            for (int step = 0; step < iter; ++step) {
                ...
                for (int i = dir ? 0 : numCollisions - 1; i >= 0 && i < numCollisions; i += dir ? 1 : -1)
        ...
        private void NotifyAllPrePhysics(float dt) {
            int numBodies = bodies.Count;
            for (int i = 0; i < numBodies; ++i) bodies[i].PrePhysics(dt);
        }
        ...
        private void BuildIslands() {
            foreach (CollisionIsland c in islands) freeCollisionIslands.Push(c);
            islands.Clear();
        ...
        private void DampAllActiveBodies() {
            int numBodies = activeBodies.Count;
            for (int i = 0; i < numBodies; ++i) activeBodies[i].DampForDeactivation();
        }
        ...
        public void Integrate(float dt) {
            ...
            if (freezingEnabled && collisions.Count != 0) {
                ...
                for (int i = 0; i < bodies.Count; i++) bodies[i].UpdateDeactivation(dt);
        ...
        public int NumPenetrationRelaxtionTimesteps {
            set { numPenetrationRelaxtionTimesteps = value; }
            get { return numPenetrationRelaxtionTimesteps; }
        }
        ...
        public bool IsShockStepEnabled {
            set { doShockStep = value; }
            get { return doShockStep; }
        }
        ...
        public Vector3 Gravity {
            ...
            set {
                ...
                if (value.X == value.Y && value.Y == value.Z) gravityAxis = -1;

                gravityAxis = 0;

                if (System.Math.Abs(value.Y) > System.Math.Abs(value.X)) gravityAxis = 1;

                float[] gravity = new float[3] {value.X, value.Y, value.Z};

                if (System.Math.Abs(value.Z) > System.Math.Abs(gravity[gravityAxis])) gravityAxis = 2;
            }
        }
        ...
        public bool EnableFreezing {
            set {
                ...
                if (!freezingEnabled) {
                    int numBodies = bodies.Count;
                    for (int i = 0; i < numBodies; ++i) bodies[i].SetActive();
                }
            }
        }
        ...
        static PhysicsSystem() {
            for (int i = 0; i < 128; ++i) freeContacts.Push(new Contact());
            for (int i = 0; i < 64; ++i) freeCollisionIslands.Push(new CollisionIsland());
        }
        ...
        private void UpdateContactCache() {
            foreach (Contact c in catchedContacts) freeContacts.Push(c);
            ...
            for (int i = collisions.Count; i-- != 0;) {
                ...
                for (int pos = 0; pos < collInfo.NumCollPts; ++pos) {
                    ...
                    Vector3 fricImpulse = collInfo.SkinInfo.Skin0.Owner.ID > skinId1 ? ptInfo.AccumulatedFrictionImpulse : -ptInfo.AccumulatedFrictionImpulse;

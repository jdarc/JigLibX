...
namespace JigLibX.Collision.Detection {
    ...
    public class CollDetectBoxStaticMesh : DetectFunctor {
        ...
        /// </summary>
        public CollDetectBoxStaticMesh() : base("BoxStaticMesh", (int) PrimitiveType.Box, (int) PrimitiveType.TriangleMesh) { }
        ...
        private void CollDetectSweep(ref CollDetectInfo info, float collTolerance, CollisionFunctor collisionFunctor) {
            ...
            if (nPositions == 1) { CollDetectBoxStaticMeshOverlap(oldBox, newBox, mesh, ref info, collTolerance, collisionFunctor); } else {
        ...
        private static int GetBoxTriangleIntersectionPoints(List<Vector3> pts, Box box, Triangle triangle, float combinationDistance) {
            ...
            for (iEdge = 0; iEdge < 3; ++iEdge) {
                ...
                if (box.SegmentIntersect(out tS, out pos, out n, s1)) AddPoint(pts, ref pos, combinationDistance * combinationDistance);
                if (box.SegmentIntersect(out tS, out pos, out n, s2)) AddPoint(pts, ref pos, combinationDistance * combinationDistance);
            }
        ...
        private static bool DoOverlapBoxTriangleTest(Box oldBox, Box newBox, ref IndexedTriangle triangle, TriangleMesh mesh, ref CollDetectInfo info, float collTolerance, CollisionFunctor collisionFunctor) {
            ...
            if (Disjoint(out testDepth, ref triNormal, newBox, ref tri, collTolerance)) return false;

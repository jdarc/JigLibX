...
namespace JigLibX.Collision.Detection {
    ...
    public class CollDetectSphereStaticMesh : DetectFunctor {
        ...
        /// Constructor
        /// </summary>
        public CollDetectSphereStaticMesh() : base("SphereStaticMesh", (int) PrimitiveType.Sphere, (int) PrimitiveType.TriangleMesh) { }
        ...
        internal static void CollDetectSphereStaticMeshSweep(BoundingSphere oldSphere, BoundingSphere newSphere, TriangleMesh mesh, CollDetectInfo info, float collTolerance, CollisionFunctor collisionFunctor) {
            ...
            if (delta.LengthSquared() < 0.25f * newSphere.Radius * newSphere.Radius) { CollDetectSphereStaticMeshOverlap(oldSphere, newSphere, mesh, info, collTolerance, collisionFunctor); } else {
                Vector3 body0Pos = info.Skin0.Owner != null ? info.Skin0.Owner.OldPosition : Vector3.Zero;
                Vector3 body1Pos = info.Skin1.Owner != null ? info.Skin1.Owner.OldPosition : Vector3.Zero;
                        ...
                        fixed (int* potentialTriangles = potTriArray) {
                            ...
                            for (int iTriangle = 0; iTriangle < numTriangles; ++iTriangle) {
                                ...
                                if (d2 < sphereToR2) {
                                    ...
                                    float depth;
                                    if (Intersection.SweptSphereTriangleIntersection(out pt, out N, out depth, oldSphere, newSphere, triangle, distToCentreOld, distToCentreNew, Intersection.EdgesToTest.EdgeAll, Intersection.CornersToTest.CornerAll)) {

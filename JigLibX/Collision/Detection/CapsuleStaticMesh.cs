...
namespace JigLibX.Collision.Detection {
    ...
    public class CollDetectCapsuleStaticMesh : DetectFunctor {
        ...
        /// Constructor
        /// </summary>
        public CollDetectCapsuleStaticMesh() : base("CapsuleStaticMesh", (int) PrimitiveType.Capsule, (int) PrimitiveType.TriangleMesh) { }
                    ...
                    fixed (int* potentialTriangles = potTriArray) {
                        ...
                        for (int iTriangle = 0; iTriangle < numTriangles; ++iTriangle) {
                            ...
                            if (d2 < capsuleTolR2) {
                                ...
                                Vector3 collisionN = d2 > JiggleMath.Epsilon ? JiggleMath.NormalizeSafe(oldSeg.GetPoint(tS) - pt) : meshTriangle.Plane.Normal;
        ...
        private void CollDetectCapsulseStaticMeshSweep(Capsule oldCapsule, Capsule newCapsule, TriangleMesh mesh, CollDetectInfo info, float collTolerance, CollisionFunctor collisionFunctor) {
            ...
            if (delta.LengthSquared() < 0.25f * newCapsule.Radius * newCapsule.Radius) { CollDetectCapsuleStaticMeshOverlap(oldCapsule, newCapsule, mesh, info, collTolerance, collisionFunctor); } else {

        ...
        public void ExtractData(List<Vector3> vertices, List<TriangleVertexIndices> indices, Model model) {
            ...
            foreach (ModelMesh mm in model.Meshes) {
                ...
                foreach (ModelMeshPart mmp in mm.MeshParts) {
                    ...
                    for (int i = 0; i != a.Length; ++i) Vector3.Transform(ref a[i], ref xform, out a[i]);
                    vertices.AddRange(a);

                    //if (mm.IndexBuffer.IndexElementSize != IndexElementSize.SixteenBits)      //XNA 4.0 change
                    if (mmp.IndexBuffer.IndexElementSize != IndexElementSize.SixteenBits) throw new Exception(string.Format("Model uses 32-bit indices, which are not supported."));
